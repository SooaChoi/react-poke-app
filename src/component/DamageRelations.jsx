...
const DamageRelations = ({damages}) => {
  ...
  useEffect(() => {
    ...
    if(arrayDamage.length === 2) {
      ...
      setDamagePokemonForm(reduceDuplicateVaules(postDamageValue(obj.from)));
    } else {
  ...
  const reduceDuplicateVaules = (props) => {
    const duplicateValues = {double_damage: '4x', half_damage: '1/4x', no_damage: '0x'};
  ...
  const postDamageValue = (props) => {
    const result =  Object.entries(props)
                          .reduce((acc, [keyName, value]) => {
                            const key = keyName;
                            const valuesOfKeyName = {double_damage: '2x', half_damage: '1/2x', no_damage: '0x'};
      ...
      {damagePokemonForm ? (
        <>
          {Object.entries(damagePokemonForm).map(([keyName, value]) => {
            const key = keyName;
            const valuesOfKeyName = {double_damage: 'Weak', half_damage: 'Resistant', no_damage: 'Immune'}
