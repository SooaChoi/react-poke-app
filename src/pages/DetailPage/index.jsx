...
const DetailPage = () => {
  ...
  if(isLoading) {
    return (
      <div className={`absolute h-auto w-auto top-1/3 -translate-x-1/2 left-1/2 z-50`}><Loading className='w-12 h-12 animate-spin text-slate-900'/></div>
    )
  }

  if(!isLoading && !pokemon) {
    return (
      <div className={`absolute h-auto w-auto top-1/3 -translate-x-1/2 left-1/2 z-50`}>Not Found</div>
    )
  }
      ...
      <div className={`${bg} w-auto h-full flex flex-col z-0 items-center justify-end relative overflow-hidden`}>
        ...
        <section className='w-full flex flex-col z-20 items-center justify-end relative h-full'>
          <div className='absolute z-30 top-6 flex items-center w-full justify-between px-2'>
            ...
            <div className='text-zinc-200 text-md font-bold'>
              #{pokemon.id.toString().padStart(3, '00')}
            </div>
          </div>
          
          <div className='relative h-auto max-w-[15.5rem] z-20 mt-6 -mb-16'>
            <img src={img} onClick={() => setIsModalOpen(true)} width="100%" height="auto" loading='lazy' alt={pokemon.name} className={`object-contain h-full`} />
          </div>
        </section>
